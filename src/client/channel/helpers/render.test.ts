...
describe("rendering a channel name", () => {
  it("renders a single channel's name from its exchange, its symbol, and the suffix", () => {
    ...
    /* A suffix is the server's own, spaces and symbols included. */
  ...
  it("renders an aggregated channel's name with the marker last, whatever the suffix", () => {
  ...
  ])("rejects a single name that cannot be rendered: %j %j", (product, suffix) => {
    expect(() => renderSingleName(product, suffix)).toThrow(VeloError);
  });
  ...
  it.each([
    ...
    [42, ""],
    ["BTC", 42],
  ])("rejects an aggregated name that cannot be rendered: %j %j", (coin, suffix) => {
    expect(() => renderAggregatedName(coin as never, suffix as never)).toThrow(VeloError);
  });

  it("says what a suffix must look like", () => {
    expect(() => renderAggregatedName("BTC", "open_interest")).toThrow(
      'a channel name suffix must be empty or start with # (got "open_interest")',
    );
  });
