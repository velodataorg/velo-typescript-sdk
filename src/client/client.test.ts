...
describe("Velo.futures.query", () => {
  ...
  it("is sealed: neither the caller's arrays nor the prepared query can change the request", async () => {
    const { velo: client, urls } = velo(ROWS_CSV);
    ...
    // the prepared query is frozen: mutating it throws instead of bypassing validation
    expect(() => (query.prepared.requests[0]!.columns as string[]).push("open_price")).toThrow(
      TypeError,
    );
  ...
  it("rejects invalid params at query construction, before any request", () => {
    ...
    const cases: unknown[] = [
      ...
      // one bucket already over the cell budget: lowering happens eagerly too
      { ...params, products: Array.from({ length: 30_000 }, (_, i) => `P${i}`) },
    ];
