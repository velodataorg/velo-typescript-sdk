...
import { decodeNewsMessage, DEFAULT_NEWS_HEARTBEAT_TIMEOUT } from "./watch.js";
import type { NewsClose, NewsDelete, NewsWatcher, NewsWatcherState } from "./watch.js";
...
describe("Velo.news.watch", () => {
  ...
  it("reports a post-open decode failure as error then close", async () => {
    ...
    expect(surfaced?.body).toBe("{not json");
