...
describe("Velo.news.stories", () => {
  ...
  it("rejects valid JSON whose response wrapper is malformed", async () => {
    for (const body of ["null", "[]", "{}", '{"stories":{}}']) {
      ...
      const error = await client.news.stories().catch((e: unknown) => e);
  ...
  it("requires every documented story field, including nullable ones", async () => {
  ...
  it("surfaces invalid JSON through the transport error", async () => {
