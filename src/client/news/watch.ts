...
export type NewsWatcherState = "idle" | "connecting" | "open" | "closed";
...
export interface NewsWatcher {
  ...
  /**
   * Opens the single owned socket and subscribes to live News.
   *
   * Concurrent calls share one connection attempt. Once closed, a watcher
   * cannot reconnect; create a new watcher instead.
   */
...
/**
 * A disconnected, one-shot controller for the live News WebSocket.
 *
 * The watcher owns one connection and deliberately never reconnects: without
 * a server cursor, reconnecting could silently lose edits or deletions.
 */
...
export function decodeNewsMessage(text: string): DecodedNewsMessage {
  ...
  try {
    ...
    throw new VeloError(`unexpected ${NEWS_WEBSOCKET_PATH} message: invalid JSON`, {
      body: text,
      cause,
    });
  }
